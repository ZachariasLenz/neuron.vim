...
func! neuron#search_content(use_cursor)
    ...
    if g:neuron_search_backend ==? 'ag'
        call fzf#vim#ag(l:query, fzf#vim#with_preview({'dir': g:neuron_dir, 'options': '--exact'}), g:neuron_fullscreen_search)
    elseif g:neuron_search_backend ==? 'rg'
        call s:ripgrep_fzf(l:query)
    endif
endf

func! s:ripgrep_fzf(query) abort
    let command_fmt = 'rg --column --line-number --no-heading --color=always --smart-case -- %s || true'
    let initial_command = printf(command_fmt, shellescape(a:query))
    let reload_command = printf(command_fmt, '{q}')
    let spec = {'dir': g:neuron_dir, 'options': ['--phony', '--query', a:query, '--bind', 'change:reload:' . reload_command]}
    call fzf#vim#grep(initial_command, 1, fzf#vim#with_preview(spec), g:neuron_fullscreen_search)
endf
...
func! neuron#edit_zettel_new(title, as_folgezettel)
    if bufname('%') != ''
        w
    endif
...
func! neuron#edit_zettel_new_from_cword()
    ...
    execute "normal! diw"
    call neuron#edit_zettel_new(l:title, 1)
endf
...
func! neuron#edit_zettel(zettel_id)
    ...
    exec 'edit ' . g:neuron_dir_zettels . a:zettel_id . g:neuron_extension
    call neuron#add_virtual_titles()
endf
...
func! neuron#tags_update(tag)
    ...
    if l:line_tags == 0
        "go to the second line, and search forwards to front matter sep
        execute "normal! gg"
        execute "normal! j"
...
func! neuron#tags_search()
    ...
    if (g:neuron_debug_enable)
        call writefile(split(l:data, "\n", 1), g:neuron_dir . 'query_tags.json')
    endif
    ...
    call fzf#run(fzf#wrap({
        \ 'options': util#get_fzf_options('Search zettel with tag #' . l:tag . ': '),
        \ 'source': l:zettel_tag_search,
        \ 'sink': function('util#edit_shrink_fzf'),
    \ }, g:neuron_fullscreen_search))
endf

" Find the neuron directory (the one with the neuron.dhall file).
func! neuron#get_neuron_dir() abort
    let l:neuron_dir = fnamemodify(expand('%:p'), ':h') . '/'

    " Fallback to using getcwd if the above gives us a relative path.
    if l:neuron_dir == './'
        let l:neuron_dir = getcwd() . '/'
    endif

    " Ensure that we're in the root neuron directory.
    if !filereadable(l:neuron_dir . 'neuron.dhall')
        " Check if neuron.dhall file is in any of the above directories. This
        " can be the case if we're already in the zettel subdirectory.
        try
            let l:dir_above = l:neuron_dir
            while l:dir_above != '/' && l:dir_above != expand('~') . '/'
                let l:dir_above = fnamemodify(l:dir_above, ':h:h') . '/'
                if filereadable(l:dir_above . 'neuron.dhall')
                    return l:dir_above
                endif
            endwhile
        catch
            " Do nothing, we're probably just not in a neuron directory.
        endtry
    endif

    " No Zettelkasten found, so let's return g:neuron_dir or the cwd.
    return l:neuron_dir
endf

" Check for any zettel subdirectories.
func! neuron#get_zettels_dir() abort
    for dir in get(g:, 'neuron_dirs_to_check', [])
        if !empty(expand(glob(g:neuron_dir . dir . '/*' . g:neuron_extension)))
             return g:neuron_dir . dir . '/'
        endif
    endfor
    return g:neuron_dir
endf
